                                ...
                                <div class="mini-image" v-for="(img,index) in imagesDraggable" :key="index">
                                    <div  class="box-img">
                                        <transition name="slide-fade">
                                            <img v-show="!loadRenderImgMini" @load="loadRenderImgMini = false"  style="margin:0px 3px;"  :src="img"  >
                                        </transition>
                                        <template v-if="loadRenderImgMini">
                                            <v-progress-circular
                                            indeterminate
                                            size="40"
                                            color="info"
                ...
                <v-container>
                    <v-row class="d-flex justify-center flex-column">
                        <v-row class="d-flex justify-center flex-row my-2" >
                            ...
                            >
                            <!-- passingIndex viene del manejo del slide actual -->
                                <v-icon>
                                ...
                                <draggable  class="gallery-draggable" v-model="imagesDraggable" group="people" @start="drag=true" @end="drag=false">
                                    <div  class="mini-image" v-for="(img,index) in imagesDraggable" :key="index">
                                            <div  @click="slide.passingIndex = index" class="box-img">
                                                <transition name="slide-fade">
                                                    <img v-show="!loadRenderImg" @load="loadRenderImg = false" ref="renderImg" style="margin:0px 3px;" :src="img"  >
                                                </transition>
                                                <template v-if="loadRenderImg">
                                                    <v-progress-circular
                                                    indeterminate
                                                    size="40"
                                                    color="info"
...
export default {
    ...
    methods:{
        ...
        deleteImage(index){
            const deleted = this.imagesDraggable
            deleted.forEach((val,i)=>{
                if(index === i){
                   if(val.split(':')[0] === 'http' || val.split(':')[0] === 'https'){
                              var del = {
                                file_name: val.split('/')[val.split('/').length - 1],
                                file_path : val.split('/')[val.split('/').length - 2],
                              }
                            this.deletedImagesServer.push(del)
    ...
    watch:{
        ...
        deletedImagesServer(val){
            this.$emit("DeletedImagesServer",val)
        },
        galleryImage :{
            handler(value){
                this.loadingDialog()
                if(!value.display){
                    if(!this.loadRenderImg){
                        this.loadRenderImg =  true
                        console.log("reseteo")
                    }
